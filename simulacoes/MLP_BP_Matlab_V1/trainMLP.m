function [Wx,Wy,MSE]=trainMLP(p,H,m,mu,alpha,X,D,epoch,MSEmin)
...
//   MSEmin: Mean square error target.
//
// Output parameters:
...
disp(D);
		...
		mse = mean(mean(E.^2));
		MSETemp(i) = mse;
		disp(['epoch = ' mtlb_num2str(i) ' mse = ' mtlb_num2str(mse)]);
		if (mse < MSEmin)
		    MSE = MSETemp(1:i);
		    return
		end
